...
self.onmessage = (event: MessageEvent) => {
    const { input, handSize, batchSize } = event.data as SimulationWorkerMessage;
    console.log(`Running simulation with ${batchSize} batches of ${handSize} hands`);

    const deck = buildDeck(input.deck);
    const sims = simulateDraw(deck, input.conditions.map(parseCondition), handSize, batchSize);
